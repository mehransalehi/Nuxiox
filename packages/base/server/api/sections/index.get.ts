...
const layerName = process.env.NUXT_LAYER || 'dentist'
...
export default defineEventHandler(async (event) => {
  ...
  const themeDir = path.resolve(process.cwd(), `packages/themes/${layerName}/app/components/sections`)
  const pkgDir = path.resolve(process.cwd(), `packages/${layerName}/app/components/sections`)
