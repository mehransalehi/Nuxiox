...
<template>
  <article v-if="data" class="space-y-8">
    <header class="space-y-3 rounded-3xl bg-sky-100 p-6 md:p-8">
    ...
    <section v-if="publicSettings?.blog?.commentsEnabled" class="space-y-4">
      ...
      <div class="rounded-3xl border border-base-200 bg-base-100 p-5 shadow-sm">
        ...
        <div class="mt-4 flex items-center gap-2">
          <button class="btn rounded-full border-none bg-orange-500 text-white hover:bg-orange-600" @click="postComment">Submit comment</button>
          <button v-if="replyTo" class="btn" @click="replyTo = null">Cancel reply</button>
        </div>
      </div>

      <div v-for="comment in data.comments" :key="comment.id" class="rounded-3xl border border-base-200 bg-base-100 p-5 shadow-sm">
